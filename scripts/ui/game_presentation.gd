...
func _ready() -> void:
	...
	ui.child_entered_tree.connect(func(node: Node) -> void:
		PresentationMetrics.bind_descendants(node))
...
func _exit_tree() -> void:
	...
	if scene_tree != null and scene_tree.root.size_changed.is_connected(_layout):
		scene_tree.root.size_changed.disconnect(_layout)
	PresentationMetrics.unbind_runtime_overlays(scene_tree)
...
func _layout() -> void:
	...
	PresentationMetrics.bind_runtime_overlays(get_tree())
